...
if ($oldversion < 2021042006)
    {
        {

            // Define field intro to be added to tab
            $table = new xmldb_table('tab');
            $field06 = new xmldb_field('listactivity', XMLDB_TYPE_TEXT, 'small', null, null, null, null, 'introformat');
            $field07 = new xmldb_field('printheading', XMLDB_TYPE_INTEGER, '1', 1, null, null, null, 'listactivity');
            
    
            // Conditionally launch add field intro
            if (!$dbman->field_exists($table, $field06))
            {
                $dbman->add_field($table, $field06);
            }
            if (!$dbman->field_exists($table, $field07))
            {
                $dbman->add_field($table, $field07);
            }
        //+ Moodle 3.0 Update
        // tab savepoint reached
        upgrade_mod_savepoint(true, 2021042006, 'tab');
    }
}
}